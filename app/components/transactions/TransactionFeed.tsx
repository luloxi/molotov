  ...
  const getEventText = (event: typeof events[0]) => {
    switch (event.type) {
      case 'mint':
        return (
          <>
            <span className={styles.address}>{truncateAddress(event.from)}</span>
            {' minted a new artwork'}
            {event.artworkTitle && <span className={styles.highlight}> &quot;{event.artworkTitle}&quot;</span>}
          </>
        );
      case 'purchase':
        return (
          <>
            <span className={styles.address}>{truncateAddress(event.to || '')}</span>
            {' purchased from '}
            <span className={styles.address}>{truncateAddress(event.from)}</span>
            {event.price && (
      ...
      case 'register':
        return (
          <>
            <span className={styles.address}>{truncateAddress(event.from)}</span>
            {' joined as an artist'}
            {event.artistName && <span className={styles.highlight}> ({event.artistName})</span>}
          </>
        );
      default:
